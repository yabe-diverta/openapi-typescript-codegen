...
export class Auth {
    public static async login(param: Parameters<typeof SpecialOperationInfo.login.method>[0]) {
        ...
        const request = SpecialOperationInfo.login.method(param);
        const rtn = await request.then(prms => prms.value() as Promise<LoginResponse>);
        const { grant_token, errors } = rtn;
    ...
    public static async createToken(param: Parameters<typeof SpecialOperationInfo.token.method>[0]) {
        ...
        const { access_token, refresh_token } = await res.then(prms => prms.value() as Promise<TokenResponse>);
